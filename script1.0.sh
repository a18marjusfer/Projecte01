#!/bin/bash
#https://github.com/a18marjusfer/Projecte01


if [ $(id -u) -eq 0 ]; then                          # 1-Make sure the script is being executed with superuser privileges.
	read -p "Introdueix el nou user : " nom          # 2-Get the username (login).
    read -p "Introdueix el nom real : " nomre        # 3-Get the real name (contents for the description field).
	...
	if [ $? -eq 0 ]; then
		...
		useradd -m -c $nomre $nom                    # 5-Create the user / 7-Set the password.
        ...
        [ $? -eq 0 ] && echo "La contrasenya s'ha establert" || echo "No s'ha pogut establir la contrasenya." # 8-Check to see if the passwd command succeeded.
